...
struct struct_control_settings { uint16_t flags; 
                        unsigned int low_current;
    ...
    switch( packet.p.command ) {
        case REPROGRAM_CODE:
         ...
         case RESET_CODE:
            ...
            control_settings.flags &= ~OVERCURRENT_FAULT;
            break;
        ...
        case COMMAND_CODE:
            ...
            else if(control_settings.flags & (OVERTEMP_FAULT | OVERCURRENT_FAULT | BADVOLTAGE_FAULT));
                                                                // no command because of overtemp or overcurrent or badvoltage
            else if((control_settings.flags & (PORT_PIN_FAULT | MAX_RUDDER_FAULT)) && packet.p.value > 1000)
                stop();                                         // no forward command if port fault
            else if((control_settings.flags & (STARBOARD_PIN_FAULT | MIN_RUDDER_FAULT)) && packet.p.value < 1000)
...
void send_loop()
{
    ...
    case 0: case 10: case 20: case 30:
        if(!low_current)
            control_settings.flags |= CURRENT_RANGE;

        packet.p.value = control_settings.flags;
        control_settings.flags &= ~REBOOTED;
